 ...
 * @author okayu
 */
public class Layer extends KASLayer
{
	/** 追加画像 */
...
//	private int height = 100;				// 高さ
	...
	private boolean loadImage(Context context, String filename, float scale)
	{
		...
		loadAnimationFile(filename);	// アニメーション定義ファイル読み込み
	...
	{
		return loadImage(Util.context, filename, Layer.scale);
	}
	...
	/**
	 * アニメーションフレームにレイヤサイズを合わせる
	 */
	public void fitToAnimationFrame()
	{
	 ...
	 * @return
	 */
	public boolean loadAdditionalImage(String filename, int dx, int dy, int sx, int sy, int sw, int sh, int opacity)
	{
		if(filename == null) return false;

		Bitmap image = ResourceManager.loadBitmap(Util.context.getResources(), filename, 1f);
	...
	{
		loadAnimationFile(imageFileName);
		if(animation != null)
			setSize(animation.width, animation.height);
	}
		...
		if(visible)
		{
			...
			else if(loadedImage)
			{
				...
				if(image != null && !image.isRecycled())
				{
					...
					rect_dst.set(x, y, x+width, y+height);
					c.drawBitmap(image, x + imageX, y + imageY, paint);
				}
			}
			int size = aimage.size();
			AdditionalImage data;
	...
	public void copyLayer(Context context, KASLayer layer, long nowTime)
	{
		...
		try
		{
			...
			{
				ai = l.aimage.get(i);
				aimage.add(new AdditionalImage(ai));
			}
		}
	...
	public void loadLayer(Context context, JSONObject json, long nowTime) throws JSONException
	{
		...
		if(json.has("imagefilename"))
		{
			...
			if(imageFileName != null && imageFileName.length() != 0)
			{
				...
				{
					loadAnimationFile(json.getString("animation"));
	...
	public void setPosByString(String pos)
	{
	...
	public int getImageWidth(){ if(image == null) return 0; else return image.getWidth(); }
	public int getImageHeight(){ if(image == null) return 0; else return image.getHeight(); }
