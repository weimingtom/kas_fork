...
class FontFace
{
	...
	/** デフォルト */
	public static final Typeface FACE_DEFAULT = Typeface.DEFAULT;
	/** デフォルト太字 */
	public static final Typeface FACE_DEFAULT_BOLD = Typeface.DEFAULT_BOLD;
	/** デフォルト斜体 */
	public static final Typeface FACE_DEFAULT_ITALIC = Typeface.create(Typeface.DEFAULT, Typeface.ITALIC);
	/** デフォルト太字斜体 */
	public static final Typeface FACE_DEFAULT_BOLD_ITALIC = Typeface.create(Typeface.DEFAULT, Typeface.BOLD_ITALIC);
	...
	public void load(String filename)
	{
		...
		{
			normal = FACE_DEFAULT;
			bold = FACE_DEFAULT_BOLD;
			italic = FACE_DEFAULT_ITALIC;
			bolditalic = FACE_DEFAULT_BOLD_ITALIC;
		}
		else
		{
			Typeface face = ResourceManager.loadTypeFace(Util.context, filename);
			normal = face;
			bold = Typeface.create(face, Typeface.BOLD);
			italic = Typeface.create(face, Typeface.ITALIC);
			bolditalic = Typeface.create(face, Typeface.BOLD_ITALIC);
		}
	}
	 ...
	 * @param src コピー元
	 */
	public void copy(FontFace src)
	{
		name = src.name;
		normal = src.normal;
		bold = src.bold;
		italic = src.italic;
		bolditalic = src.bolditalic;
	}
...
//	private int height = 100;						// 高さ
	...
	/** （デフォルト）フォント */
	public FontFace defFontFace = FontFace.DEFAULT;
	/** （デフォルト）フォントサイズ */
	...
	/** フォント */
	private FontFace fontFace = FontFace.DEFAULT;
	/** フォントサイズ */
	...
	public void onDraw(Context context, Canvas c, long nowTime)
	{
		if(visible)
		{
			...
			super.onDraw(context, c, nowTime);
	...
	public void addMessage(String s)
	{
		...
		for(int i = 0; i < len; i++)
		{
			...
			else
			{
				...
				// 追加
				line.add(ch, textX, textY, pitch, fontFace, fontSize, bold, italic, color, shadow, shadowColor, edge, edgeColor);

				if(vertical)
					textY += (int)fontSize + pitch;
				else
				{
					chWidth = line.getLastCharWidth();
					textX += chWidth + pitch;
				}
	 ...
	 * @param face
	 */
	public FontFace loadFontFace(String face)
	{
...
//		String rubyoffset = elm.get("rubyoffset");
	...
	public void setDefFont(HashMap<String, String> elm)
	{
		...
		if(face != null) defFontFace = loadFontFace(face);
		...
		if(edge != null && Util.isBoolean(edge))		defEdge = (Boolean.parseBoolean(edge));
	...
	public void copyLayer(Context context, KASLayer layer, long nowTime)
	{
		...
		defFontFace.copy(m.defFontFace);
	...
	public JSONObject saveLayer() throws JSONException
	{
		...
		json.put("deffontface", defFontFace.name);
	...
	public void loadLayer(Context context, JSONObject json, long nowTime) throws JSONException
	{
		...
		if(json.has("deffontface"))		defFontFace		= loadFontFace(json.getString("deffontface"));
...
//		clearMessage();
	...
	{
		this.fontFace = loadFontFace(face);
	}
	...
	{
		fontSize = size;
		setLineSize((int)size); // linesizeの補正
	}
