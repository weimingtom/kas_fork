...
class SystemButton extends KASPlugin
{
	...
	/** 表ボタンが本来表示中かどうか */
	protected boolean foreseen;
	/** 裏ボタンが本来表示中かどうか */
	protected boolean backseen;
	...
	public SystemButton()
	{
		...
		{
			fore[i].visible = foreseen = true;
			back[i].visible = backseen = true;
		}
...
//	}
	...
	public void onMessageHiddenStateChanged(boolean hidden)
	{
		...
		else
		{
			...
			for(int i = 0; i < buttonCount; i++)
			{
				fore[i].visible = foreseen;
				back[i].visible = backseen;
			}
		}
	}
	...
	public void run(HashMap<String, String> elm)
	{
		...
		if(visible_s != null)
		{
			...
			if(layers == fore) foreseen = visible;
			else backseen = visible;
		}
	...
	{
		json.put("forevisible", foreseen);
		json.put("backvisible", backseen);
	...
	{
		if(!data.has("systembuttondata"))
		{
			for(int i = 0; i < buttonCount; i++)
			{
				fore[i].visible = foreseen = false;
				fore[i].setEnabled(true);
				back[i].visible = backseen = false;
				back[i].setEnabled(true);
			}
		}
		else
		{
			...
			if(toback) // 裏画面へのロード時
			{
				...
				for(int i = 0; i < buttonCount; i++)
				{
					back[i].visible = backseen = backvisible;
			...
			else	// 通常ロード
			{
				...
				for(int i = 0; i < buttonCount; i++)
				{
					fore[i].visible = foreseen = forevisible;
